...
// TODO: что тут будут делать неявно определенные конструкторы и деструктор?
// конструктор будет инициализировать члены класса с помощью их соответствующих конструкторов по умолчанию
// компилятор также генерирует неявный деструктор по умолчанию, который будет автоматически вызываться при выходе из области видимости
class C {
    MyString s;
    Rect a;
};
...
int main()
{
    ...
    {
        cout << "\n--task 1.2--\n\n";
        Rect rect; // компилятор создает конструктор при ее отсутствии, которы по сути ничего не делает
    }
    ...
    {
        cout << "\n--task 1.3--\n\n";
        Rect rect(1, 2, 3, 4); // пользовательский конструктор
    }
    ...
    {
        cout << "\n--task 1.4--\n\n";
        Rect rect1(1, 2, 3, 4); // инициализация
        Rect rect2 = rect1; // копирование присваиванием (вызывается конструктор копирования неявно)
        Rect rect3(rect1); // конструктор копирования
    }
     ...
     * случае?
     * 
     * дестркутор в нашем случае пустой, т.к наши данные примитивные, а в 
     * общем случае он освобождает выделенные ресурсы и вызывается автоматически, 
    ...
    {
        cout << "\n--task 1.6--\n\n";
        Rect r1; // по умолчанию
        Rect *pR = new Rect(1,2,1,2); // пользовательский, так еще и динамич
        {
            Rect r2(r1); // неявное копирование
            Rect arRect[2]; // дефолтный констр для двух эл массива
            for(int i = 0; i < 3; i++)
            {
                static Rect r3 (i,i,i,i) ; // один раз пользовательский
                Rect r4(*pR); // копирование каждую итерацию
                Rect r5(i,i,i,i); // пользователбский каждую итерацию
            }
        }
        delete pR; // очистка памяти // TODO: вызывается ли тут деструктор? Да
    }
    ...
    // { нельзя обращаться к приватным полям напрямую
    //     Rect r1;
    //     std::cout << r1.m_top << std::endl;
    // } 
     ...
     * пользуется только публичным интерфейсом, за который несет отвественность
     ...
     *
     * В этом примере и далее мы будем *предполагать*, что "бюракратичный"
     ...
     * 1. привязка к классу
     * 2. синтаксис вызова (операторы "." или "->")
     * 3. наследование (подклассы)
     * 4. инкапсуляция (закрытость)
     */
     ...
     * который осуществляет паралельный перенос по следующей логике: 
    ...
    {
        cout << "\n--task 1.10--\n\n";
        Rect r1(-3, 12, -5, 4), r2(3, -12, 5, -4); // конструкторы инициализации 
        Rect r3 = bounding_rect(r1, r2); // TODO: какие конструкторы вызываются тут? 
        //создаются копии этих эл(т.к передаются элементы, а не ссылка), следовательно копирующие конструкторы 
        print_rect(r3);
    } 
     ...
     * блоке превратиться в настояший код и решить задачу.
     */

    {
        cout << "\n--task 2.1--\n\n";
        // инициализация бочек
        Barrel alch(10.0, 9.6);  
        Barrel water(10.0, 0.0); 
        double cupVolume = 1.0;  
        int iteration = 0;

        // цикл переливаний
        while (alch.getAlcoholConc() > 50.0) {
            iteration++;
            alch.overflowFromTo(water, cupVolume);
            water.overflowFromTo(alch, cupVolume);
        }
...

    {
        ...
        double val = M.get(m-1, n-1);
        M.set(m-1, n-1, 9.99);
        int height = M.get_height();
        int width = M.get_width();

        M.add_in_place(M1);
        Matrix resM = M.multiply(M2);
    }
...

    {
        ...
        // получаем и устанавливаем символы
        cout << "symbol: " << str2.get(3) << "\n";
        str2.set(3, 'g'); // заменяем 'l' на 'g'
        cout << "after: ";
        str2.print();

        // замена строки
        str2.set_new_string("qwe asd zxc");
        cout << "new row: ";
        str2.print();
    ...
    catch (const std::exception& problem) { // TODO: зачем это? а) ловить "out of range" в get и set
        std::cerr << "err: " << problem.what() << "\n";
    }
...

    {
        ...
        MyString s4 = s3; // тут траблы(нужно выделить память)
     }
...

    {
        ...
        cout << "input ur text for bubbleCat: ";
        input.read_line();
        cout << "ur result: \n";
     ...
     *
     * Убедитель в том, что при изменении отдельных файлов программы происходит
     ...
     * высоту и ширину.
     *
     * Измените реализацию класса `Rect` соотвествующим образом, оставляя в
